...
public interface IPage<T> extends Serializable {
     ...
     */
    default boolean optimizeJoinOfCountSql() { return true; }
     ...
     */
    default Long maxLimit() { return null; }
    
    /**
     * 当前分页总页数
     */
    default long getPages() { if (getSize() == 0) { return 0L; } long pages = getTotal() / getSize(); if (getTotal() % getSize() != 0) { pages++; } return pages; }
     ...
     */
    default String countId() { return null; }
    
    PageType pageType();
    
    IPage<T> pageType(PageType pageType);
    
    public static enum PageType { OFFSET, ORDERS }
    
    boolean isFirst();
    
    IPage<T> isFirst(boolean isFirst);
}
