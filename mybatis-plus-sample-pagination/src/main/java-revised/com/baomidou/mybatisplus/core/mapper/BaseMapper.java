...
public interface BaseMapper<T> extends Mapper<T> {
     ...
     */
    default int deleteByMap(Map<String, Object> columnMap) { return this.delete(Wrappers.<T>query().allEq(columnMap)); }
     ...
     */
    @Deprecated
    default int deleteBatchIds(@Param(Constants.COLL) Collection<?> idList) { return deleteByIds(idList); }
     ...
     * @since 3.5.7
     */
    default int deleteByIds(@Param(Constants.COLL) Collection<?> idList) { return deleteByIds(idList, true); }
    ...
    default int deleteByIds(@Param(Constants.COLL) Collection<?> collections, boolean useFill) {
        ...
        if (useFill && tableInfo.isWithLogicDelete() && tableInfo.isWithUpdateFill()) { params.put(Constants.MP_FILL_ET, tableInfo.newInstance()); }
     ...
     * @since 3.5.4
     */
    default int update(@Param(Constants.WRAPPER) Wrapper<T> updateWrapper) { return update(null, updateWrapper); }
     ...
     * @deprecated 3.5.8
     */
    default List<T> selectBatchIds(@Param(Constants.COLL) Collection<? extends Serializable> idList) { return selectByIds(idList); }
     ...
     */
    @Deprecated
    default void selectBatchIds(@Param(Constants.COLL) Collection<? extends Serializable> idList, ResultHandler<T> resultHandler) { selectByIds(idList, resultHandler); }
     ...
     * @param columnMap 表字段 map 对象
     */
    default List<T> selectByMap(Map<String, Object> columnMap) { return this.selectList(Wrappers.<T>query().allEq(columnMap)); }
     ...
     * @since 3.5.4
     */
    default void selectByMap(Map<String, Object> columnMap, ResultHandler<T> resultHandler) { this.selectList(Wrappers.<T>query().allEq(columnMap), resultHandler); }
     ...
     */
    default T selectOne(@Param(Constants.WRAPPER) Wrapper<T> queryWrapper) { return this.selectOne(queryWrapper, true); }
    ...
    default T selectOne(@Param(Constants.WRAPPER) Wrapper<T> queryWrapper, boolean throwEx) {
        ...
        if (size == 1) {
            ...
            if (throwEx) { throw new TooManyResultsException("Expected one result (or null) to be returned by selectOne(), but found: " + size); }
            return list.get(0);
        }
        return null;
    }
    ...
    default <P extends IPage<T>> P selectPage(P page, @Param(Constants.WRAPPER) Wrapper<T> queryWrapper) {
        ...
        if (page.getRecords().size() > 0) {
            ...
            T                 t = page.getRecords().get(page.getRecords().size() - 1);
            try {
                for (OrderItem o : page.orders()) { Object v = b.getProperty(t, o.getColumn()); o.setValue(v); System.err.printf("%s = %s%n", o.getColumn(), o.getValue()); }
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
     ...
     * @since 3.5.7
     */
    default List<BatchResult> insertOrUpdate(Collection<T> entityList) { return insertOrUpdate(entityList, Constants.DEFAULT_BATCH_SIZE); }
